...
class PDFProcessor:
    ...
    def _remove_header_seal_scan_pdf(self, input_path, output_path):
        ...
        for page_num in range(len(doc)):
            ...
            # 创建新页面：使用原始页面的物理尺寸（point），而非渲染像素数，
            # 否则页面会被渲染分辨率（1.5倍）放大，导致后续叠加的水印相对显小
            page_rect = page.rect
    ...
    def _remove_seal_scan_pdf(self, input_path, output_path):
        ...
        for page_num in range(len(doc)):
            ...
            # 创建新页面：使用原始页面的 point 尺寸，而非渲染像素尺寸，
            # 避免因 1.5 倍渲染矩阵导致页面物理尺寸被放大
            page_rect = page.rect
            img_rect = fitz.Rect(0, 0, page_rect.width, page_rect.height)
            new_page = new_doc.new_page(width=page_rect.width, height=page_rect.height)
            new_page.insert_image(img_rect, stream=img_bytes)
    ...
    def _remove_annots_from_page_object(self, page):
        ...
        try:
            ...
            # 定义页面上部区域（红头文字区域）
            # 红头文字占页面上端70-80%，所以我们认为距顶部小于60%的是上部区域
            # PDF坐标系：y=0在底部，y=page_height在顶部
            # 上部区域：y > page_height * 0.4（距底部大于40%）
                    ...
                    if rect_match:
                        ...
                        if len(coords) >= 4:
                            ...
                            if y_center > upper_area_y_threshold:
                                in_upper_area = True
                                print(f"[DEBUG] 注释xref={xref}在页面上部（y中心={y_center:.1f}, 距顶部={from_top:.1f}），跳过")
                            else:
                                in_middle_or_bottom = True
                                print(f"[DEBUG] 注释xref={xref}在页面中下部（y中心={y_center:.1f}, 距底部={from_bottom:.1f}）")
            ...
            # 如果发现中下部的签名字段或公章，直接删除整个Annots键
            if has_seal_in_middle_or_bottom:
                print(f"[DEBUG] 发现中下部的签名字段/公章，删除整个/Annots键")
    ...
    def _convert_text_to_image_pdf(self, input_path, output_path):
        ...
        for page_num in range(len(doc)):
            page = doc[page_num]
            # 记录原始页面的物理尺寸（单位：点），新页面必须保持这个尺寸，
            # 否则后续叠加水印等操作使用的 pt 字号会与页面实际比例不匹配
            ...
            # 创建新页面：使用原始点尺寸而非像素尺寸，图片会被自动缩放以铺满页面，
            # 保证页面的物理大小和坐标系统与原始PDF一致
